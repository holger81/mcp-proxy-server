...
# Usage:
#   MAIL_MCP_VERSION=v0.4.5 ./install-mail-mcp-release.sh
#   MAIL_MCP_VERSION=latest ./install-mail-mcp-release.sh   # GitHub "latest" release (not a git tag)
# Run inside the container (appuser) or on host with DATA_ROOT=/path/to/volume.

set -e
VERSION="${MAIL_MCP_VERSION:?Set MAIL_MCP_VERSION e.g. v0.4.5 or latest}"
...
if [ "$VERSION" = "latest" ]; then
  URL="https://github.com/tecnologicachile/mail-mcp/releases/latest/download/${ARCHIVE}"
else
